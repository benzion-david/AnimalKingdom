...
public class CritterModel {
	...
	protected static int simulationCount;
	...
	public void add(int number, Class<? extends Critter> critter) {
		...
		for (int i = 0; i < number; i++) {
			...
			try {
				...
				System.out.println("ERROR: " + critter + " does not have" + " the appropriate constructor.");
				System.exit(1);
				return;
			} catch (Exception e) {
				System.out.println("ERROR: " + critter + " threw an " + " exception in its constructor.");
				System.exit(1);
				return;
			}
			int x, y;
		...
		for (int i = 0; i < list.length; i++) {
			...
			if (move == Critter.Action.LEFT)
				...
				if (inBounds(p2) && grid[p2.x][p2.y] != null && grid[p2.x][p2.y].getClass() != next.getClass()
						&& !locked.contains(grid[p2.x][p2.y]) && (hadHopped || Math.random() >= HOP_ADVANTAGE)) {
	...
	private static class Info implements CritterInfo {
		...
		public Info(Critter.Neighbor[] neighbors, Critter.Direction d, boolean[] neighborThreats) {
